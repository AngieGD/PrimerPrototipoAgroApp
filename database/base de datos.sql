DROP database if exists AgroAppF;
CREATE database IF NOT EXISTS AgroAppF;
use AgroAppF;
...
CREATE TABLE IF NOT EXISTS tipos_matenimiento (
  ...
  tipos_matenimientocol VARCHAR(45) NULL,
...
CREATE TABLE IF NOT EXISTS mantenimentos (
  ...
  fk_trabajador_manteni VARCHAR(50) NOT NULL,
...
CREATE TABLE IF NOT EXISTS subterreno_has_tipo_cultivos (
  ...
  id_subterreno_has_tipo_cultivoscol VARCHAR(45) NOT NULL,
...
CREATE TABLE IF NOT EXISTS facturas_has_cargas (
  ...
  CONSTRAINT fk_facturas_has_cargas_facturas1
    ...
    REFERENCES `AgroAppF`.`facturas` (`idfacturas`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_facturas_has_cargas_cargas1`
    FOREIGN KEY (`fk_cargas`)
    REFERENCES `AgroAppF`.`cargas` (`idcargas`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
